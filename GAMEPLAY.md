...
- **Dr. Null (The Villain):** A mad scientist coordinating robotic attacks from his command center. He has spiked white hair, a menacing expression with angled eyebrows, a cybernetic glowing green monocle eye on the left, a glowing red exposed eye on the right, and a menacing grin showing his teeth. In Level 6 (Mixed Mastery Boss Wave), Dr. Null enters the battlefield directly as the active boss, wearing a retro CRT computer monitor casing on his chest that displays the math problem. The handheld controller he usually holds is hidden, and his arms are moved to the sides, ensuring the screen is completely unobstructed.
...
- **Creeping Mechanics:** Glitch-Bots do not advance towards the heroes unless the player selects a wrong answer. Questions change immediately on both correct and incorrect answers.
...
- **Victory Condition:** The player wins/masters the active level by reducing the target's health from 100% to 0%. Levels 1-5 target Glitch-Bots (losing 25% health per correct answer, requiring 4 correct answers). Level 6 targets Dr. Null (losing 5% health per correct answer, requiring 20 correct answers). Upon victory, the Mastery modal is displayed, and the level mastery sound effect (`level-mastered.mp3`) plays.
- **Defeated Condition (Game Over):** If the Glitch-Bot or Dr. Null reaches `100%` progress and crosses the checkered finish line, the defense is breached, resulting in a Game Over. This displays the Defense Breached overlay and plays the level failure sound effect (`level-failed.mp3`).
- **Completion Modals:** Both the Level Mastered (Victory) and Defense Breached (Game Over) overlay screens display the final score.

### Level 6: Mixed Mastery Boss Wave
- **Active Hero & Math Problems:** Equations are selected at random from Levels 1 to 5, along with their respective active heroes. Correct answers trigger the attack animation and sound effect of the hero active for that specific puzzle (Dash for single-digit or basic addition, Titan for subtraction, Aero for carry addition or borrow subtraction).
- **Target Boss:** Dr. Null (wearing a CRT computer monitor casing displaying the math problem).
- **Boss Health & Scaling:**
  - Dr. Null starts with `100%` health.
  - A correct answer inflicts 5% damage (requiring 20 net correct answers to defeat him).
  - A wrong answer heals him by 5% (up to a maximum of 100% health).
  - A wrong answer also advances him by `15%` progress towards the finish line.
